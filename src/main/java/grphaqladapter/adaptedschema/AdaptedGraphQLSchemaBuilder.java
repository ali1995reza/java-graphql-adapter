...
import grphaqladapter.adaptedschema.exceptions.MultipleGraphqlTopLevelTypeException;
...
import grphaqladapter.adaptedschema.mapping.mapper.ClassMapper;
...
public final class AdaptedGraphQLSchemaBuilder {
    ...
    public synchronized AdaptedGraphQLSchema build() {
        ...
        mappedClasses = addStringAndBoolean(mappedClasses);
        ...
        List<DiscoveredElement> allElements = CollectionUtils.separateToImmutableList(discoveredElements.values(), DiscoveredElement.class);
    ...
    public DiscoveredElement buildDiscoveredElement(MappedElement element, GraphQLNamedSchemaElement schemaElement) {
        ...
        if (cls.mappedType().isObjectType() || cls.mappedType().isTopLevelType()) {
            ...
            return new DiscoveredObjectTypeImpl((MappedObjectTypeClass) cls, schemaElement.getName(), (GraphQLObjectType) schemaElement);
            ...
            return new DiscoveredScalarTypeImpl((MappedScalarClass) cls, schemaElement.getName(), (GraphQLScalarType) schemaElement);
        }
    ...
    private void makeImmutable(List<DiscoveredElement> elements) {
        elements.stream().filter(element -> element instanceof DiscoveredTypeImpl)
                ...
                .forEach(DiscoveredTypeImpl::makeImmutable);
    }
