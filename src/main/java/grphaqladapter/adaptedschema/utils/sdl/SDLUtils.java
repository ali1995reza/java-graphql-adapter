...
package grphaqladapter.adaptedschema.utils.sdl;
...
public class SDLUtils {
    ...
    public static String getSchemaObjectAsSDL(GraphQLSchema schema) {
        ...
        buffer.append(SCHEMA_SIGNATURE).append(SPACE).append(STRUCTURE_START).append(DOUBLE_NEW_LINE);
        if (schema.getQueryType() != null) {
            buffer.append(INNER_OFFSET).append(QUERY_SIGNATURE).append(DOUBLE_DOT).append(SPACE)
                    .append(schema.getQueryType().getName()).append(DOUBLE_NEW_LINE);
        }
        if (schema.getMutationType() != null) {
            buffer.append(INNER_OFFSET).append(MUTATION_SIGNATURE).append(DOUBLE_DOT).append(SPACE)
                    .append(schema.getMutationType().getName()).append(DOUBLE_NEW_LINE);
        }
        if (schema.getSubscriptionType() != null) {
            buffer.append(INNER_OFFSET).append(SUBSCRIPTION_SIGNATURE).append(DOUBLE_DOT).append(SPACE)
                    .append(schema.getSubscriptionType().getName()).append(DOUBLE_NEW_LINE);
        }
        return buffer.append(STRUCTURE_END).toString();
    }
    ...
    public static String toSDL(GraphQLArgument argument) {
        ...
        if (argument.hasSetDefaultValue()) {
            buffer.append(EQUAL).append(toSDL(argument.getArgumentDefaultValue()));
        }
        if (!CollectionUtils.isEmpty(argument.getAppliedDirectives())) {
            buffer.append(SPACE).append(toSDLOneLine(argument.getAppliedDirectives()));
        }
        return buffer.toString();
    }
    ...
    public static String toSDL(GraphQLInputObjectField field) {
        ...
        if (field.hasSetDefaultValue()) {
            buffer.append(EQUAL).append(toSDL(field.getInputFieldDefaultValue().getValue()));
        }
    ...
    private static String getDescriptionForFieldArgument(GraphQLArgument argument) {
        ...
        return new StringBuffer().append(DOUBLE_QUOTATION)
                .append(SPACE)
