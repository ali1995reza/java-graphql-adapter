...
public class CustomDirectiveArgumentFooListValueParser implements ValueParser<int[], List<Foo>> {
    @Override
    public List<Foo> parse(int[] input, TypeInformation descriptor, ValueParsingContext context) {
        List<Foo> list = new ArrayList();
        for (int i = 0; i < input.length; i++) {
            list.add(new Foo().setIntValue(input[i]));
        }
        return list;
    }
}
